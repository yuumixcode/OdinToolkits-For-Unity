...
{
    [Summary("OdinToolkits 文件夹路径定位器")]
    public sealed class OdinToolkitsLookup : ScriptableObject
    {
        [DisplayAsString(FontSize = 22, Overflow = false)]
        [TempComposite]
        public string WarningCn1 => "不要删除这个资源文件！";

        [DisplayAsString(FontSize = 15, Overflow = false)]
        [TempComposite]
        public string WarningCn2 => "该资源文件用于定位 OdinToolkits 文件夹路径，防止整体移动 OdinToolkits 文件夹时出现错误。";

        [DisplayAsString(FontSize = 22, Overflow = false)]
        [TempComposite]
        public string WarningEn1 => "Don't Delete this file !";
