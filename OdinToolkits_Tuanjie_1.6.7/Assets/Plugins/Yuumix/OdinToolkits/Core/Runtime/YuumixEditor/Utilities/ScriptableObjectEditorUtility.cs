...
namespace YuumixEditor
{
    ...
    {
        const string GET_ASSET_AND_DELETE_EXTRA_KEY = nameof(GetAssetAndDeleteExtra) + ".RelativeFolderPath";
        ...
        public static T GetAssetAndDeleteExtra<T>(string relativeFolderPath = "") where T : ScriptableObject
        {
            ...
            if (string.IsNullOrEmpty(relativeFolderPath))
            {
                ...
                EditorPrefs.SetString(GET_ASSET_AND_DELETE_EXTRA_KEY, relativeFolderPath);
            }

            PathEditorUtility.CreateDirectoryRecursivelyInAssets(relativeFolderPath);
            var filePath = relativeFolderPath + "/" + typeof(T).Name + "[AutoGenerated].asset";
        ...
        [DidReloadScripts]
        static void AutoLogWarning()
        {
            if (!EditorPrefs.HasKey(GET_ASSET_AND_DELETE_EXTRA_KEY))
            {
                return;
            }

            var relativeFolderPath = EditorPrefs.GetString(GET_ASSET_AND_DELETE_EXTRA_KEY);
            YuumixLogger.LogWarning($"没有找到对应的 ScriptableObject 资源，且没有设置路径。自动生成资源在 {relativeFolderPath}");
            EditorPrefs.DeleteKey(GET_ASSET_AND_DELETE_EXTRA_KEY);
        }
    }
}
#endif
