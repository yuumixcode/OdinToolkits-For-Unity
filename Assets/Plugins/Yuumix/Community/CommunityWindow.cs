...
namespace Yuumix.OdinToolkits.Editor.Core
{
    ...
    public class CommunityWindow : OdinEditorWindow
    {
        ...
        [MenuItem(OdinToolkitsWindowMenuItems.CommunityMenuItemName, false,
            OdinToolkitsWindowMenuItems.CommunityPriority)]
        public static void ShowWindow()
        {
            var window = GetWindow<CommunityWindow>();
            if (InspectorMultiLanguageManagerSO.IsChinese)
            {
                window.titleContent = new GUIContent(OdinToolkitsWindowMenuItems.CommunityWindowNameCn);
            }
            else if (InspectorMultiLanguageManagerSO.IsEnglish)
            {
                window.titleContent = new GUIContent(OdinToolkitsWindowMenuItems.CommunityWindowNameEn);
            }
