    ...
    public class EventAnalysisDataFactory : IEventAnalysisDataFactory
    {
        public EventAnalysisData CreateFromEventInfo(EventInfo eventInfo, Type type)
        {
            var eventData = new EventAnalysisData
            {
                ...
                memberAccessModifierType = TypeAnalyzerHelper.GetEventAccessModifierType(eventInfo),
                returnType = TypeAnalyzerHelper.GetReadableEventReturnType(eventInfo),
                isObsolete = eventInfo.IsDefined(typeof(ObsoleteAttribute)),
                name = eventInfo.Name
            };
            
            TypeAnalyzerHelper.IsStaticEvent(eventInfo, eventData);
            var keyword = "";
