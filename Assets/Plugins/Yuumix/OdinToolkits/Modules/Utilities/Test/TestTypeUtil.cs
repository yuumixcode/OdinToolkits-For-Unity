...
namespace Yuumix.OdinToolkits.Modules.Utilities.Test
{
    ...
    public class TestTypeUtil : SerializedMonoBehaviour
    {
        ...
        public void TestGetTypeDeclaration()
        {
            if (testData)
            {
                returnList = testData.TypeListForTest.Select(TypeUtil.GetTypeDeclaration)
                    .ToList();
            }
            else
            {
                returnList = TypesForTest.Select(TypeUtil.GetTypeDeclaration)
                    .ToList();
            }
        }
