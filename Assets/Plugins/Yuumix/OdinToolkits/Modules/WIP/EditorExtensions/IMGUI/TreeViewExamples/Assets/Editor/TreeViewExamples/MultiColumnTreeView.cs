...
namespace Yuumix.OdinToolkits.Modules.LearnArchive.EditorExtensions.IMGUI.TreeViewExamples.Assets.Editor.TreeViewExamples
{
    internal class MultiColumnTreeView : TreeViewWithTreeModel<MyTreeElement>
    {
        ...
        int GetIcon1Index(TreeViewItem<MyTreeElement> item) =>
            (int)(Mathf.Min(0.99f, item.data.floatValue1) * s_TestIcons.Length);

        int GetIcon2Index(TreeViewItem<MyTreeElement> item) => Mathf.Min(item.data.text.Length, s_TestIcons.Length - 1);
