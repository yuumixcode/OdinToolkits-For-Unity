...
{
    [MultiLanguageComment("Odin SerializedMonoBehaviour 单例抽象类。", "Abstract class for an Odin SerializedMonoBehaviour singleton.")]
    public abstract class OdinSingleton<T> : SerializedMonoBehaviour where T : OdinSingleton<T>
    {
        bool _isInitialized;
        static T _instance;

        [MultiLanguageComment("获取单例实例，如果实例不存在则创建一个新的实例。", "Gets the singleton instance. If the instance does not exist, a new instance is created.")]
        ...
        [MultiLanguageComment("OnDestroy 方法，当单例实例被销毁时调用。", "OnDestroy method, called when the singleton instance is destroyed.")]
