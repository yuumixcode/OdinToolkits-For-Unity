...
namespace Yuumix.OdinToolkits
{
    ...
    [MultiLanguageComment("单例助手类，用于无法继承单例抽象类的类实现单例模式。", "Singleton assistant class, used for classes that cannot inherit from the singleton abstract class to implement the singleton pattern.")]
    public sealed class SingletonAssistant<T> where T : class, ISingleton
    {
        ...
        [MultiLanguageComment("获取单例实例。", "Gets the singleton instance.")]
