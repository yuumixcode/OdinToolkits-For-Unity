    ...
    public class XMLCodePart
    {
        ...
        public string SummaryValue
        {
            get
            {
                ...
                if (match.Success)
                {
                    var summaryContent = match.Groups[1]
                        .Value
                        .Trim();
                    ...
                    cleanedSummaryContent = Regex.Replace(cleanedSummaryContent, @"^\s*$\r?\n", "",
                        RegexOptions.Multiline);
                    // 压缩连续的空白字符
                    cleanedSummaryContent = Regex.Replace(cleanedSummaryContent, @"\s+", " ")
                        .Trim();
        ...
        public string SummaryAttributeText
        {
            get
            {
                ...
                var indent = Regex.Match(xml, @"^\s*")
                    .Value;
                var attr = nameof(SummaryAttribute)
                    .Replace("Attribute", "");
        ...
        public string RemovedFirstSummaryAttributeCode
        {
            get
            {
                ...
                var match = Regex.Match(code, @"(?m)(?:^|\s)\s*\[" + attr + @"\(""(?<content>[\s\S]*?)""\)\]",
                    RegexOptions.Multiline);
                if (match.Success)
                {
        ...
        public string GetReplaceOutput() =>
            RemovedSummaryXml + SummaryAttributeText + RemovedFirstSummaryAttributeCode;
